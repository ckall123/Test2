...
from objects.spawner import Spawner, OBJECT_NAMES
...
class XArm6GymEnv(gym.Env):
    ...
    - 獎勵：幾何 + 版面整齊 + VLM 分數；撞桌扣分並截斷 episode
    """
    ...
    def __init__(self,
        ...
        self.arm_bounds = arm_bounds  # None -> 由 Spawner 使用自己的 Arm_* 常數
        self.spawner = Spawner(arm_bounds=self.arm_bounds)

        self.object_names = object_names if object_names is not None else OBJECT_NAMES
    ...
    def step(self, action: np.ndarray):
        ...
        reward += penalty_touch_table(self.tf_buffer)

        # 7) 終止條件（步數上限或撞桌）
        terminated = False
        truncated = (self.step_count >= self.max_steps) or touched_table(self.tf_buffer)

        info = {
            'gripper_position': self.gripper_angle,
            'vlm_score': self._last_vlm_score,
            'touched_table': bool(truncated and self.step_count < self.max_steps),
        }
    ...
    def reset(self, *, seed=None, options=None):
        ...
        for obj in self.objects:
            try:
                self.spawner.delete_object(obj.name)
            except Exception:
                pass
        self.objects = []
        time.sleep(0.10)

        # 重新 spawn 最多 N_OBJECTS 個物件
        names = random.sample(self.object_names, k=min(len(self.object_names), N_OBJECTS))
        for base_name in names:
            pos = self.spawner.get_safe_random_position()
            actual_name, ok = self.spawner.spawn_object(base_name, pos)
            if ok:
                self.objects.append(SimObject(name=actual_name, position=pos))
