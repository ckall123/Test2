...
class XArm6GymEnv(gym.Env):
    ...
    def __init__(self, max_steps=200, vlm_interval=20, vlm_prompt=".", camera_topic="/camera/image_raw",
        ...
        self.vlm_interval = int(vlm_interval)   # ← 修正
        ...
        class _VLMAdapter:
            def score(self, bgr_img, prompt: str):
                return get_vlm_score(bgr_img, prompt=prompt)

        self.vlm_scorer = _VLMAdapter()
        # 旋鈕保存
    ...
    def _ee_pos(self, joints: np.ndarray) -> np.ndarray:
        try:
            ...
            tf = self.tf_buffer.lookup_transform(
                "base_link",
                "ee_link",
                Time(),  # 代表 'now'，也可以用 Time(seconds=0)
    ...
    def _reward(self, obs: dict) -> float:
        ...
        # ----- 這裡開始是新的 VLM 穩定版 -----
        vlm_score = self._last_vlm_score  # 預設沿用上一分數
        ...
        if need_vlm and (self.latest_image is not None):
            try:
                img = self.latest_image  # BGR uint8
                if img is not None and getattr(img, "size", 0) > 0:
                    # 你要用多少輸入尺寸都可；維持原先 IMG_W/IMG_H 也行
                    resized = cv2.resize(img, (IMG_W, IMG_H), interpolation=cv2.INTER_LINEAR)
                    vlm_score = float(self.vlm_scorer.score(resized, prompt=self.vlm_prompt))
                    vlm_score = max(0.0, min(1.0, vlm_score))  # 邊界保護
                    # 成功才覆寫 & 歸零計數
                ...
                if getattr(self, "node", None):
                    try:
                        self.node.get_logger().warn(f"VLM scoring failed: {e}")
                    except Exception:
                        pass
        r_vlm = self._last_vlm_score
        reward = 0.7 * (0.6 * r_geom + 0.4 * r_layout) + 0.3 * r_vlm
        return float(reward)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.step_count = 0
        self._step_since_vlm = 0          # ← 加這行
        self.current_joint_state[:] = 0.0
        self.gripper_angle = self.grip_max
        self._send_gripper_goal(self.gripper_angle)
        self.objects = [np.array([0.5, 0.0, 0.05], np.float32),
                        np.array([0.6, 0.0, 0.05], np.float32)]
        return self._obs(), {}
    ...
    def close(self):
        if hasattr(self, "node"):
            ...
            try:
                ...
                pass
        # 不在這裡呼叫 rclpy.shutdown()，由最外層 train.py / eval.py 負責
