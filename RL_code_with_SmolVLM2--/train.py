...
KNOBS = {
    ...
    "OLLAMA_VLM": os.environ.get("OLLAMA_VLM", "qwen2.5vl"),
    # 影像下採樣（降低 ReplayBuffer 佔用）
...
def make_env(max_steps: int, train: bool = True):
    def _thunk():
        ...
        # wrappers：Spawn → 影像下採樣 → AddState → VLM shaping → Monitor → TimeLimit
        env = SpawnOnResetWrapper(env)
        env = ImageResizeWrapper(env, size=(KNOBS["IMAGE_SIZE"], KNOBS["IMAGE_SIZE"]))
        env = AddStateKeyWrapper(env)
        env = VLMRewardWrapper(env, interval=KNOBS["VLM_INTERVAL"], coeff=0.5,
