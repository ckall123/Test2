...
class TidyCubeEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        if joint_msg is None:
            print("[ERROR] Gripper joint_msg is None")
        ...
        if joint_msg:
            ...
            try:
                ...
                print("[ERROR] KeyError in joint positions, using zeros")
                ordered_pos = [0.0] * 6
        else:
            ordered_pos = [0.0] * 6
        ...
        if pos is None:
            print("[ERROR] FK returned None")

        gripper_val = self.gripper.get_state()
        if gripper_val is None:
            print("[ERROR] Gripper state is None")
    ...
    def step(self, action):
        ...
        joint_angles = self.ik.compute_ik(target.tolist())
        self.controller.send_joint_positions(joint_angles, duration=0.5)
        self.controller.control_gripper(action[3])
