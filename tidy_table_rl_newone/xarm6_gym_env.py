...
class XArm6Env(gym.Env):
    ...
    def reset(self, *, seed: int | None = None, options: Dict[str, Any] | None = None):
        ...
        self.vlm_throttle.reset(self._score_vlm_rgb())
    ...
    def _apply_action(self, action: np.ndarray) -> None:
        ...
        q_tar = q_now + dq
        g_cmd = float(np.clip((a[-1] + 1.0) * 0.5, 0.0, 1.0))  # [-1,1] → [0,1]
        self.controller.plan_and_execute(q_tar, g_cmd)
    ...
    def _rel_vec_to_target(self) -> np.ndarray:
        ...
        if rel is None or not np.isfinite(rel).all():
    ...
    def _tcp_heading_cos_to_target(self) -> float:
        ...
        tcp_pos, tcp_yaw = self.tracker.tcp_pose()
        if tcp_pos is None or tcp_yaw is None:
            return 0.0
        tgt_pos, _ = self.tracker.object_pose(self.current_target)
        if tgt_pos is None:
            return 0.0
