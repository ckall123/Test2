...
class MoveIt2Controller:
    ...
    def move_to_pose(self, pos_xyz: list[float], quat_xyzw: list[float], duration: float | None = None) -> bool:
        """利用 IK 把姿態轉 6R 關節後下達。"""
        code, sol6, _seed = solve_ik(pos_xyz, quat_xyzw, group=self.arm_group, tip=self.eef_link, node=self.node)
        if code != 1:
    ...
    def get_current_pose(self) -> tuple[list[float] | None, list[float] | None]:
        ...
        return get_current_pose(node=self.node, base="link_base", tip=self.eef_link)
    ...
    def control_gripper(self, g_open: float, duration: float | None = None) -> bool:
        ...
        - 當 g_open < 0.5 並雙指同時接觸到同一物時 → attach
        - 當 g_open >= 0.5 並目前有附著 → detach
        """
