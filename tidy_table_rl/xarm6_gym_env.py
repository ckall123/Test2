...
class XArm6Env(gym.Env):
    ...
    def _build_state(self) -> np.ndarray:
        ...
        obj_map = {o["name"]: np.concatenate([o["pos"], [o["yaw"]]]) for o in objs}

        obj_features = []
        for name in self.object_names:
            if name in obj_map:
                obj_features.append(obj_map[name])
            else:
                obj_features.append(np.zeros(4, dtype=np.float32))

        obj_flat = np.concatenate(obj_features, axis=0)
