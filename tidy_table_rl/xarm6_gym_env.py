...
from attach_detach_client import AttachDetachClient
...
class XArmEnvConfig:
    ...
    x_range: Tuple[float, float] = (0.25, 0.55)
    ...
    v_thresh: float = 0.01
    envelope_dims: Tuple[float, float, float] = (0.03, 0.03, 0.03)
...
class XArm6Env(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        if self.held_object and grip_cmd < self.cfg.open_thresh:
            ...
            if self.contact_monitor.is_stable(candidate, self.cfg.contact_frames, self.cfg.v_thresh, self.cfg.envelope_dims):
