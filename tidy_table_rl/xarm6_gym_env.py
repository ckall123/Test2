# xarm6_gym_env.py
# ✅ 改寫目標：符合「關節為主」版完整規格 + 整合 Spawner（修正版）
...
class XArm6Env(gym.Env):
    ...
    def __init__(self, node, executor, cfg: XArmEnvConfig = XArmEnvConfig()):
        ...
        self.spawner = Spawner(node, executor)  # ✅ 修正：傳入 executor
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.spawner.spawn_random_objects(count=self.k_objects)  # ✅ 改成正確方法
        self.object_names = load_target_names()[: self.k_objects]  # ✅ 重新同步物件名單
    ...
    def _build_state(self, contact_flag: float) -> np.ndarray:
        ...
        q_norm = np.clip(2 * (q - q_limits[:, 0]) /
                         (q_limits[:, 1] - q_limits[:, 0]) - 1.0, -1.0, 1.0)
        ...
        for name in self.object_names:
            if name in obj_map:
                ...
                x = np.clip(2 * (pos[0] - x_min) /
                            (x_max - x_min) - 1.0, -1.0, 1.0)
                y = np.clip(2 * (pos[1] - y_min) /
                            (y_max - y_min) - 1.0, -1.0, 1.0)
                z = np.clip(2 * (pos[2] - z_min) /
                            (z_max - z_min) - 1.0, -1.0, 1.0)  # ✅ 修正 z clip
                yaw_sin = np.sin(yaw)
                yaw_cos = np.cos(yaw)
