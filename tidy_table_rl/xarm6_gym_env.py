...
class XArm6Env(gym.Env):
    ...
    def _build_state(self) -> np.ndarray:
        ...
        q_limits = self.controller.joint_limits  # 來源：moveit_controller.JOINT_LIMITS
        q_norm = np.clip(2 * (q - q_limits[:, 0]) / (q_limits[:, 1] - q_limits[:, 0]) - 1.0, -1.0, 1.0)

        # --- 夾爪正規化 ---
        g = float(self.controller.get_gripper_state())
        g_norm = np.array([2.0 * g - 1.0], dtype=np.float32)
        ...
        z_min, z_max = z_center - 0.1, z_center + 0.1
        ...
        # --- 接觸旗標 ---
        ok, _ = self.contact_monitor.check_dual_contact(self.object_names)
        contact_flag = np.array([1.0 if ok else 0.0], dtype=np.float32)

        return np.concatenate([q_norm, g_norm, obj_flat, hist, contact_flag], axis=0)
